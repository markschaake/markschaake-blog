...
Churn measures that - it's one way to quantify slop. Files modified and then modified again within a short window are a reasonable proxy for _rework_ - code that didn't stick the first time. Not every re-edit is slop - legitimate features cause refactors too - but persistent churn on recently touched files is a signal worth watching. The git integration already tracks which commits came from which sessions, so computing churn against the same timeline as the ratio is straightforward. I want that number minimal and trending down - as models improve, as the harness gets tighter, as the planning process learns what level of specification produces output that holds.

A rising machine:wall ratio paired with flat or declining churn is the signal of genuine progress - more parallel execution, and the output holds up. A rising ratio paired with rising churn means the parallelism is producing work that doesn't survive contact with reality. Agents stepping on each other, specifications undercooked, review too thin to catch problems before they compound. That pattern looks productive on any throughput dashboard. The churn data shows it isn't.

The planning-to-burst cycle should predict this - sessions starting with thorough decomposition producing lower-churn commits downstream, sessions that skip planning and go straight to parallel execution producing high throughput and high churn. I don't have enough data yet to confirm the pattern, but it's the hypothesis the measurement is designed to test.

<RatioVsChurnChart />

---

In a team context, these behavioral signals look less like a dashboard and more like a conversation prompt. The shifts are gradual - a machine:wall ratio lifting from 1.0 to 1.2 over three weeks as someone starts running concurrent sessions for the first time. Planning share increasing as the investment in specification starts to precede execution. An engineer whose edit-loop frequency is high - the same files getting revised repeatedly within a session - might be at a different point in the transition than one whose sessions are heavy on planning tools and light on edits. Churn rate layered on top of the ratio sharpens the picture further - two engineers with identical machine:wall ratios look very different when one has 8% churn and the other has 25%. The data doesn't diagnose anything, but it makes patterns visible that would otherwise stay hidden behind the same commit log.
